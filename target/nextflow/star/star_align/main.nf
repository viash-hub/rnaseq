...
meta = [
  ...
  "argument_groups" : [
    {
      ...
      "arguments" : [
        ...
        {
          "type" : "file",
          "name" : "--star_align_bam_transcriptome",
          "default" : [
            "$id.star_aligned.transcriptome.bam"
          ],
          "must_exist" : true,
          "create_parent" : true,
          "required" : false,
      ...
      "setup" : [
        {
          "type" : "docker",
          "run" : [
            "apt-get update && \\\\\napt-get install -y --no-install-recommends ${PACKAGES} && \\\\\nwget --no-check-certificate https://github.com/samtools/samtools/releases/download/1.18/samtools-1.18.tar.bz2 && \\\\\ntar -xjf samtools-1.18.tar.bz2 && \\\\\nrm samtools-1.18.tar.bz2 && \\\\\ncd samtools-1.18 && \\\\\n./configure && \\\\\nmake && \\\\\nmake install && \\\\\ncd && \\\\\nwget --no-check-certificate https://github.com/alexdobin/STAR/archive/refs/tags/2.7.11a.zip && \\\\\nunzip 2.7.11a.zip && \\\\\ncp STAR-2.7.11a/bin/Linux_x86_64_static/STAR /usr/local/bin \n"
          ],
          "env" : [
            "PACKAGES build-essential gcc g++ autoconf automake make wget zlib1g-dev unzip perl libbz2-dev liblzma-dev libcurl4-gnutls-dev libssl-dev libncurses5-dev bzip2 gawk"
          ]
        }
      ]
    }
  ...
  "build_info" : {
    "config" : "/home/runner/work/rnaseq.vsh/rnaseq.vsh/src/star/star_align/config.vsh.yaml",
    "runner" : "nextflow",
    "engine" : "docker",
    "output" : "/home/runner/work/rnaseq.vsh/rnaseq.vsh/target/nextflow/star/star_align",
...
$( if [ ! -z ${VIASH_PAR_STAR_INDEX+x} ]; then echo "${VIASH_PAR_STAR_INDEX}" | sed "s#'#'\\"'\\"'#g;s#.*#par_star_index='&'#" ; else echo "# par_star_index="; fi )
$( if [ ! -z ${VIASH_PAR_STAR_IGNORE_SJDBGTF+x} ]; then echo "${VIASH_PAR_STAR_IGNORE_SJDBGTF}" | sed "s#'#'\\"'\\"'#g;s#.*#par_star_ignore_sjdbgtf='&'#" ; else echo "# par_star_ignore_sjdbgtf="; fi )
$( if [ ! -z ${VIASH_PAR_SEQ_PLATFORM+x} ]; then echo "${VIASH_PAR_SEQ_PLATFORM}" | sed "s#'#'\\"'\\"'#g;s#.*#par_seq_platform='&'#" ; else echo "# par_seq_platform="; fi )
$( if [ ! -z ${VIASH_PAR_SEQ_CENTER+x} ]; then echo "${VIASH_PAR_SEQ_CENTER}" | sed "s#'#'\\"'\\"'#g;s#.*#par_seq_center='&'#" ; else echo "# par_seq_center="; fi )
$( if [ ! -z ${VIASH_PAR_EXTRA_STAR_ALIGN_ARGS+x} ]; then echo "${VIASH_PAR_EXTRA_STAR_ALIGN_ARGS}" | sed "s#'#'\\"'\\"'#g;s#.*#par_extra_star_align_args='&'#" ; else echo "# par_extra_star_align_args="; fi )
...
$( if [ ! -z ${VIASH_PAR_STAR_ALIGN_BAM+x} ]; then echo "${VIASH_PAR_STAR_ALIGN_BAM}" | sed "s#'#'\\"'\\"'#g;s#.*#par_star_align_bam='&'#" ; else echo "# par_star_align_bam="; fi )
$( if [ ! -z ${VIASH_PAR_STAR_ALIGN_BAM_TRANSCRIPTOME+x} ]; then echo "${VIASH_PAR_STAR_ALIGN_BAM_TRANSCRIPTOME}" | sed "s#'#'\\"'\\"'#g;s#.*#par_star_align_bam_transcriptome='&'#" ; else echo "# par_star_align_bam_transcriptome="; fi )
$( if [ ! -z ${VIASH_PAR_LOG_FINAL+x} ]; then echo "${VIASH_PAR_LOG_FINAL}" | sed "s#'#'\\"'\\"'#g;s#.*#par_log_final='&'#" ; else echo "# par_log_final="; fi )
...
meta["defaults"] = [
  ...
  "container" : {
    "registry" : "ghcr.io",
    "image" : "data-intuitive/star/star_align",
    "tag" : "main_build"
  },
  "tag" : "$id"
}'''),
