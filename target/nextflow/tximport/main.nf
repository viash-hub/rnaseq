...
meta = [
  ...
  "build_info" : {
    "config" : "/home/runner/work/rnaseq.vsh/rnaseq.vsh/src/tximport/config.vsh.yaml",
    "runner" : "nextflow",
    "engine" : "docker",
    "output" : "/home/runner/work/rnaseq.vsh/rnaseq.vsh/target/nextflow/tximport",
...
$( if [ ! -z ${VIASH_PAR_SALMON_QUANT_RESULTS+x} ]; then echo "${VIASH_PAR_SALMON_QUANT_RESULTS}" | sed "s#'#'\\"'\\"'#g;s#.*#par_salmon_quant_results='&'#" ; else echo "# par_salmon_quant_results="; fi )
$( if [ ! -z ${VIASH_PAR_TX2GENE_TSV+x} ]; then echo "${VIASH_PAR_TX2GENE_TSV}" | sed "s#'#'\\"'\\"'#g;s#.*#par_tx2gene_tsv='&'#" ; else echo "# par_tx2gene_tsv="; fi )
$( if [ ! -z ${VIASH_PAR_VERSIONS+x} ]; then echo "${VIASH_PAR_VERSIONS}" | sed "s#'#'\\"'\\"'#g;s#.*#par_versions='&'#" ; else echo "# par_versions="; fi )
$( if [ ! -z ${VIASH_PAR_TPM_GENE+x} ]; then echo "${VIASH_PAR_TPM_GENE}" | sed "s#'#'\\"'\\"'#g;s#.*#par_tpm_gene='&'#" ; else echo "# par_tpm_gene="; fi )
$( if [ ! -z ${VIASH_PAR_COUNTS_GENE+x} ]; then echo "${VIASH_PAR_COUNTS_GENE}" | sed "s#'#'\\"'\\"'#g;s#.*#par_counts_gene='&'#" ; else echo "# par_counts_gene="; fi )
$( if [ ! -z ${VIASH_PAR_COUNTS_GENE_LENGTH_SCALED+x} ]; then echo "${VIASH_PAR_COUNTS_GENE_LENGTH_SCALED}" | sed "s#'#'\\"'\\"'#g;s#.*#par_counts_gene_length_scaled='&'#" ; else echo "# par_counts_gene_length_scaled="; fi )
$( if [ ! -z ${VIASH_PAR_COUNTS_GENE_SCALED+x} ]; then echo "${VIASH_PAR_COUNTS_GENE_SCALED}" | sed "s#'#'\\"'\\"'#g;s#.*#par_counts_gene_scaled='&'#" ; else echo "# par_counts_gene_scaled="; fi )
$( if [ ! -z ${VIASH_PAR_TPM_TRANSCRIPT+x} ]; then echo "${VIASH_PAR_TPM_TRANSCRIPT}" | sed "s#'#'\\"'\\"'#g;s#.*#par_tpm_transcript='&'#" ; else echo "# par_tpm_transcript="; fi )
$( if [ ! -z ${VIASH_PAR_COUNTS_TRANSCRIPT+x} ]; then echo "${VIASH_PAR_COUNTS_TRANSCRIPT}" | sed "s#'#'\\"'\\"'#g;s#.*#par_counts_transcript='&'#" ; else echo "# par_counts_transcript="; fi )
