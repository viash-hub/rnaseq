...
meta = [
      ...
      "setup" : [
        {
          ...
          "env" : [
            "PACKAGES build-essential gcc g++ autoconf automake make wget zlib1g-dev unzip autoconf make gcc perl libbz2-dev liblzma-dev libcurl4-gnutls-dev libssl-dev libncurses5-dev bzip2 gawk"
          ]
        }
      ]
    }
  ...
  "build_info" : {
    "config" : "/home/runner/work/rnaseq.vsh/rnaseq.vsh/src/samtools/samtools_index/config.vsh.yaml",
    "runner" : "nextflow",
    "engine" : "docker",
    "output" : "/home/runner/work/rnaseq.vsh/rnaseq.vsh/target/nextflow/samtools/samtools_index",
...
$( if [ ! -z ${VIASH_PAR_BAM_CSI_INDEX+x} ]; then echo "${VIASH_PAR_BAM_CSI_INDEX}" | sed "s#'#'\\"'\\"'#g;s#.*#par_bam_csi_index='&'#" ; else echo "# par_bam_csi_index="; fi )
$( if [ ! -z ${VIASH_PAR_VERSIONS+x} ]; then echo "${VIASH_PAR_VERSIONS}" | sed "s#'#'\\"'\\"'#g;s#.*#par_versions='&'#" ; else echo "# par_versions="; fi )
$( if [ ! -z ${VIASH_PAR_OUTPUT_BAI+x} ]; then echo "${VIASH_PAR_OUTPUT_BAI}" | sed "s#'#'\\"'\\"'#g;s#.*#par_output_bai='&'#" ; else echo "# par_output_bai="; fi )
$( if [ ! -z ${VIASH_PAR_OUTPUT_CSI+x} ]; then echo "${VIASH_PAR_OUTPUT_CSI}" | sed "s#'#'\\"'\\"'#g;s#.*#par_output_csi='&'#" ; else echo "# par_output_csi="; fi )
...
meta["defaults"] = [
  ...
  "container" : {
    "registry" : "ghcr.io",
    "image" : "data-intuitive/samtools/samtools_index",
    "tag" : "main_build"
  },
  "tag" : "$id"
}'''),
