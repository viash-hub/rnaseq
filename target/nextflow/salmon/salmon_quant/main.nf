...
meta = [
  ...
  "build_info" : {
    "config" : "/home/runner/work/rnaseq.vsh/rnaseq.vsh/src/salmon/salmon_quant/config.vsh.yaml",
    "runner" : "nextflow",
    "engine" : "docker",
    "output" : "/home/runner/work/rnaseq.vsh/rnaseq.vsh/target/nextflow/salmon/salmon_quant",
...
$( if [ ! -z ${VIASH_PAR_SALMON_INDEX+x} ]; then echo "${VIASH_PAR_SALMON_INDEX}" | sed "s#'#'\\"'\\"'#g;s#.*#par_salmon_index='&'#" ; else echo "# par_salmon_index="; fi )
$( if [ ! -z ${VIASH_PAR_GTF+x} ]; then echo "${VIASH_PAR_GTF}" | sed "s#'#'\\"'\\"'#g;s#.*#par_gtf='&'#" ; else echo "# par_gtf="; fi )
$( if [ ! -z ${VIASH_PAR_TRANSCRIPT_FASTA+x} ]; then echo "${VIASH_PAR_TRANSCRIPT_FASTA}" | sed "s#'#'\\"'\\"'#g;s#.*#par_transcript_fasta='&'#" ; else echo "# par_transcript_fasta="; fi )
$( if [ ! -z ${VIASH_PAR_ALIGNMENT_MODE+x} ]; then echo "${VIASH_PAR_ALIGNMENT_MODE}" | sed "s#'#'\\"'\\"'#g;s#.*#par_alignment_mode='&'#" ; else echo "# par_alignment_mode="; fi )
$( if [ ! -z ${VIASH_PAR_LIB_TYPE+x} ]; then echo "${VIASH_PAR_LIB_TYPE}" | sed "s#'#'\\"'\\"'#g;s#.*#par_lib_type='&'#" ; else echo "# par_lib_type="; fi )
$( if [ ! -z ${VIASH_PAR_EXTRA_SALMON_QUANT_ARGS+x} ]; then echo "${VIASH_PAR_EXTRA_SALMON_QUANT_ARGS}" | sed "s#'#'\\"'\\"'#g;s#.*#par_extra_salmon_quant_args='&'#" ; else echo "# par_extra_salmon_quant_args="; fi )
...
$( if [ ! -z ${VIASH_PAR_JSON_INFO+x} ]; then echo "${VIASH_PAR_JSON_INFO}" | sed "s#'#'\\"'\\"'#g;s#.*#par_json_info='&'#" ; else echo "# par_json_info="; fi )
...
meta["defaults"] = [
  ...
  "container" : {
    "registry" : "ghcr.io",
    "image" : "data-intuitive/salmon/salmon_quant",
    "tag" : "main_build"
  },
  "tag" : "$id"
}'''),
