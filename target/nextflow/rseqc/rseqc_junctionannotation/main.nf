// rseqc_junctionannotation main_build
// 
...
meta = [
  ...
  "test_resources" : [
    ...
    {
      "type" : "file",
      "path" : "testData/unit_test_resources/Pairend_StrandSpecific_51mer_Human_hg19.bam"
    },
    {
      "type" : "file",
      "path" : "testData/unit_test_resources/hg19_RefSeq.bed.gz"
    }
  ],
  "info" : {
    "migration_info" : {
  ...
  "build_info" : {
    "config" : "/home/runner/work/rnaseq.vsh/rnaseq.vsh/src/rseqc/rseqc_junctionannotation/config.vsh.yaml",
    "runner" : "nextflow",
    "engine" : "docker",
    "output" : "/home/runner/work/rnaseq.vsh/rnaseq.vsh/target/nextflow/rseqc/rseqc_junctionannotation",
...
$( if [ ! -z ${VIASH_PAR_REFGENE+x} ]; then echo "${VIASH_PAR_REFGENE}" | sed "s#'#'\\"'\\"'#g;s#.*#par_refgene='&'#" ; else echo "# par_refgene="; fi )
$( if [ ! -z ${VIASH_PAR_MAP_QUAL+x} ]; then echo "${VIASH_PAR_MAP_QUAL}" | sed "s#'#'\\"'\\"'#g;s#.*#par_map_qual='&'#" ; else echo "# par_map_qual="; fi )
$( if [ ! -z ${VIASH_PAR_MIN_INTRON+x} ]; then echo "${VIASH_PAR_MIN_INTRON}" | sed "s#'#'\\"'\\"'#g;s#.*#par_min_intron='&'#" ; else echo "# par_min_intron="; fi )
$( if [ ! -z ${VIASH_PAR_VERSIONS+x} ]; then echo "${VIASH_PAR_VERSIONS}" | sed "s#'#'\\"'\\"'#g;s#.*#par_versions='&'#" ; else echo "# par_versions="; fi )
$( if [ ! -z ${VIASH_PAR_OUTPUT_LOG+x} ]; then echo "${VIASH_PAR_OUTPUT_LOG}" | sed "s#'#'\\"'\\"'#g;s#.*#par_output_log='&'#" ; else echo "# par_output_log="; fi )
$( if [ ! -z ${VIASH_PAR_OUTPUT_PLOT_R+x} ]; then echo "${VIASH_PAR_OUTPUT_PLOT_R}" | sed "s#'#'\\"'\\"'#g;s#.*#par_output_plot_r='&'#" ; else echo "# par_output_plot_r="; fi )
$( if [ ! -z ${VIASH_PAR_OUTPUT_JUNCTION_BED+x} ]; then echo "${VIASH_PAR_OUTPUT_JUNCTION_BED}" | sed "s#'#'\\"'\\"'#g;s#.*#par_output_junction_bed='&'#" ; else echo "# par_output_junction_bed="; fi )
$( if [ ! -z ${VIASH_PAR_OUTPUT_JUNCTION_INTERACT+x} ]; then echo "${VIASH_PAR_OUTPUT_JUNCTION_INTERACT}" | sed "s#'#'\\"'\\"'#g;s#.*#par_output_junction_interact='&'#" ; else echo "# par_output_junction_interact="; fi )
$( if [ ! -z ${VIASH_PAR_OUTPUT_JUNCTION_SHEET+x} ]; then echo "${VIASH_PAR_OUTPUT_JUNCTION_SHEET}" | sed "s#'#'\\"'\\"'#g;s#.*#par_output_junction_sheet='&'#" ; else echo "# par_output_junction_sheet="; fi )
$( if [ ! -z ${VIASH_PAR_OUTPUT_SPLICE_EVENTS_PLOT+x} ]; then echo "${VIASH_PAR_OUTPUT_SPLICE_EVENTS_PLOT}" | sed "s#'#'\\"'\\"'#g;s#.*#par_output_splice_events_plot='&'#" ; else echo "# par_output_splice_events_plot="; fi )
$( if [ ! -z ${VIASH_PAR_OUTPUT_SPLICE_JUNCTIONS_PLOT+x} ]; then echo "${VIASH_PAR_OUTPUT_SPLICE_JUNCTIONS_PLOT}" | sed "s#'#'\\"'\\"'#g;s#.*#par_output_splice_junctions_plot='&'#" ; else echo "# par_output_splice_junctions_plot="; fi )
...
meta["defaults"] = [
  ...
  "container" : {
    "registry" : "ghcr.io",
    "image" : "data-intuitive/rseqc/rseqc_junctionannotation",
    "tag" : "main_build"
  },
  "tag" : "$id"
}'''),
