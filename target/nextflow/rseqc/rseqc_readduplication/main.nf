// rseqc_readduplication main_build
// 
...
meta = [
  ...
  "build_info" : {
    "config" : "/home/runner/work/rnaseq.vsh/rnaseq.vsh/src/rseqc/rseqc_readduplication/config.vsh.yaml",
    "runner" : "nextflow",
    "engine" : "docker",
    "output" : "/home/runner/work/rnaseq.vsh/rnaseq.vsh/target/nextflow/rseqc/rseqc_readduplication",
...
$( if [ ! -z ${VIASH_PAR_READ_COUNT_UPPER_LIMIT+x} ]; then echo "${VIASH_PAR_READ_COUNT_UPPER_LIMIT}" | sed "s#'#'\\"'\\"'#g;s#.*#par_read_count_upper_limit='&'#" ; else echo "# par_read_count_upper_limit="; fi )
...
$( if [ ! -z ${VIASH_PAR_OUTPUT_DUPLICATION_RATE_PLOT_R+x} ]; then echo "${VIASH_PAR_OUTPUT_DUPLICATION_RATE_PLOT_R}" | sed "s#'#'\\"'\\"'#g;s#.*#par_output_duplication_rate_plot_r='&'#" ; else echo "# par_output_duplication_rate_plot_r="; fi )
$( if [ ! -z ${VIASH_PAR_OUTPUT_DUPLICATION_RATE_PLOT+x} ]; then echo "${VIASH_PAR_OUTPUT_DUPLICATION_RATE_PLOT}" | sed "s#'#'\\"'\\"'#g;s#.*#par_output_duplication_rate_plot='&'#" ; else echo "# par_output_duplication_rate_plot="; fi )
$( if [ ! -z ${VIASH_PAR_OUTPUT_DUPLICATION_RATE_MAPPING+x} ]; then echo "${VIASH_PAR_OUTPUT_DUPLICATION_RATE_MAPPING}" | sed "s#'#'\\"'\\"'#g;s#.*#par_output_duplication_rate_mapping='&'#" ; else echo "# par_output_duplication_rate_mapping="; fi )
$( if [ ! -z ${VIASH_PAR_OUTPUT_DUPLICATION_RATE_SEQUENCE+x} ]; then echo "${VIASH_PAR_OUTPUT_DUPLICATION_RATE_SEQUENCE}" | sed "s#'#'\\"'\\"'#g;s#.*#par_output_duplication_rate_sequence='&'#" ; else echo "# par_output_duplication_rate_sequence="; fi )
...
meta["defaults"] = [
  ...
  "container" : {
    "registry" : "ghcr.io",
    "image" : "data-intuitive/rseqc/rseqc_readduplication",
    "tag" : "main_build"
  },
  "tag" : "$id"
}'''),
