...
meta = [
  ...
  "test_resources" : [
    ...
    {
      "type" : "file",
      "path" : "testData/unit_test_resources/Pairend_StrandSpecific_51mer_Human_hg19.bam.bai"
    },
    {
      "type" : "file",
  ...
  "build_info" : {
    "config" : "/home/runner/work/rnaseq.vsh/rnaseq.vsh/src/rseqc/tin/config.vsh.yaml",
    "runner" : "nextflow",
    "engine" : "docker",
    "output" : "/home/runner/work/rnaseq.vsh/rnaseq.vsh/target/nextflow/rseqc/rseqc_tin",
...
$( if [ ! -z ${VIASH_PAR_BAI_INPUT+x} ]; then echo "${VIASH_PAR_BAI_INPUT}" | sed "s#'#'\\"'\\"'#g;s#.*#par_bai_input='&'#" ; else echo "# par_bai_input="; fi )
$( if [ ! -z ${VIASH_PAR_REFGENE+x} ]; then echo "${VIASH_PAR_REFGENE}" | sed "s#'#'\\"'\\"'#g;s#.*#par_refgene='&'#" ; else echo "# par_refgene="; fi )
$( if [ ! -z ${VIASH_PAR_MINIMUM_COVERAGE+x} ]; then echo "${VIASH_PAR_MINIMUM_COVERAGE}" | sed "s#'#'\\"'\\"'#g;s#.*#par_minimum_coverage='&'#" ; else echo "# par_minimum_coverage="; fi )
$( if [ ! -z ${VIASH_PAR_SAMPLE_SIZE+x} ]; then echo "${VIASH_PAR_SAMPLE_SIZE}" | sed "s#'#'\\"'\\"'#g;s#.*#par_sample_size='&'#" ; else echo "# par_sample_size="; fi )
$( if [ ! -z ${VIASH_PAR_SUBTRACT_BACKGROUND+x} ]; then echo "${VIASH_PAR_SUBTRACT_BACKGROUND}" | sed "s#'#'\\"'\\"'#g;s#.*#par_subtract_background='&'#" ; else echo "# par_subtract_background="; fi )
$( if [ ! -z ${VIASH_PAR_VERSIONS+x} ]; then echo "${VIASH_PAR_VERSIONS}" | sed "s#'#'\\"'\\"'#g;s#.*#par_versions='&'#" ; else echo "# par_versions="; fi )
$( if [ ! -z ${VIASH_PAR_OUTPUT_TIN_SUMMARY+x} ]; then echo "${VIASH_PAR_OUTPUT_TIN_SUMMARY}" | sed "s#'#'\\"'\\"'#g;s#.*#par_output_tin_summary='&'#" ; else echo "# par_output_tin_summary="; fi )
$( if [ ! -z ${VIASH_PAR_OUTPUT_TIN+x} ]; then echo "${VIASH_PAR_OUTPUT_TIN}" | sed "s#'#'\\"'\\"'#g;s#.*#par_output_tin='&'#" ; else echo "# par_output_tin="; fi )
...
meta["defaults"] = [
  ...
  "container" : {
    "registry" : "ghcr.io",
    "image" : "data-intuitive/rseqc/rseqc_tin",
    "tag" : "main_build"
  },
  "tag" : "$id"
}'''),
