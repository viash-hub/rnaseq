...
meta = [
  ...
  "functionality" : {
    ...
    "resources" : [
      {
        ...
        "path" : "script.sh",
        "is_executable" : true,
        "parent" : "file:/home/nirmayi/data_intuitive/rnaseq.vsh/src/rseqc/innerdistance/"
      }
    ],
    ...
    "test_resources" : [
      ...
      {
        "type" : "file",
        "path" : "testData/minimal_test/Pairend_nonStrandSpecific_36mer_Human_hg19.bam",
        "parent" : "file:///home/nirmayi/data_intuitive/rnaseq.vsh/"
      },
      {
        "type" : "file",
        "path" : "testData/minimal_test/hg19_rRna.bed",
        "parent" : "file:///home/nirmayi/data_intuitive/rnaseq.vsh/"
      }
    ],
    "info" : {
  ...
  "info" : {
    ...
    "platform" : "nextflow",
    "output" : "/home/nirmayi/data_intuitive/rnaseq.vsh/target/nextflow/rseqc/rseqc_innerdistance",
...
$( if [ ! -z ${VIASH_PAR_SAMPLE_SIZE+x} ]; then echo "${VIASH_PAR_SAMPLE_SIZE}" | sed "s#'#'\\"'\\"'#g;s#.*#par_sample_size='&'#" ; else echo "# par_sample_size="; fi )
$( if [ ! -z ${VIASH_PAR_MAP_QUAL+x} ]; then echo "${VIASH_PAR_MAP_QUAL}" | sed "s#'#'\\"'\\"'#g;s#.*#par_map_qual='&'#" ; else echo "# par_map_qual="; fi )
$( if [ ! -z ${VIASH_PAR_LOWER_BOUND_SIZE+x} ]; then echo "${VIASH_PAR_LOWER_BOUND_SIZE}" | sed "s#'#'\\"'\\"'#g;s#.*#par_lower_bound_size='&'#" ; else echo "# par_lower_bound_size="; fi )
$( if [ ! -z ${VIASH_PAR_UPPER_BOUND_SIZE+x} ]; then echo "${VIASH_PAR_UPPER_BOUND_SIZE}" | sed "s#'#'\\"'\\"'#g;s#.*#par_upper_bound_size='&'#" ; else echo "# par_upper_bound_size="; fi )
$( if [ ! -z ${VIASH_PAR_STEP_SIZE+x} ]; then echo "${VIASH_PAR_STEP_SIZE}" | sed "s#'#'\\"'\\"'#g;s#.*#par_step_size='&'#" ; else echo "# par_step_size="; fi )
$( if [ ! -z ${VIASH_PAR_OUTPUT_STATS+x} ]; then echo "${VIASH_PAR_OUTPUT_STATS}" | sed "s#'#'\\"'\\"'#g;s#.*#par_output_stats='&'#" ; else echo "# par_output_stats="; fi )
$( if [ ! -z ${VIASH_PAR_OUTPUT_DIST+x} ]; then echo "${VIASH_PAR_OUTPUT_DIST}" | sed "s#'#'\\"'\\"'#g;s#.*#par_output_dist='&'#" ; else echo "# par_output_dist="; fi )
$( if [ ! -z ${VIASH_PAR_OUTPUT_FREQ+x} ]; then echo "${VIASH_PAR_OUTPUT_FREQ}" | sed "s#'#'\\"'\\"'#g;s#.*#par_output_freq='&'#" ; else echo "# par_output_freq="; fi )
...
meta["defaults"] = [
  ...
  "container" : {
    "registry" : "ghcr.io",
    "image" : "data-intuitive/rseqc/rseqc_innerdistance",
    "tag" : "dev"
  },
  "tag" : "$id"
}'''),
