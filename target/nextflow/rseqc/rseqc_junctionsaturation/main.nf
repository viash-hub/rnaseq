// rseqc_junctionsaturation main_build
// 
...
meta = [
  ...
  "build_info" : {
    "config" : "/home/runner/work/rnaseq.vsh/rnaseq.vsh/src/rseqc/rseqc_junctionsaturation/config.vsh.yaml",
    "runner" : "nextflow",
    "engine" : "docker",
    "output" : "/home/runner/work/rnaseq.vsh/rnaseq.vsh/target/nextflow/rseqc/rseqc_junctionsaturation",
...
$( if [ ! -z ${VIASH_PAR_SAMPLING_PERCENTILE_LOWER_BOUND+x} ]; then echo "${VIASH_PAR_SAMPLING_PERCENTILE_LOWER_BOUND}" | sed "s#'#'\\"'\\"'#g;s#.*#par_sampling_percentile_lower_bound='&'#" ; else echo "# par_sampling_percentile_lower_bound="; fi )
$( if [ ! -z ${VIASH_PAR_SAMPLING_PERCENTILE_UPPER_BOUND+x} ]; then echo "${VIASH_PAR_SAMPLING_PERCENTILE_UPPER_BOUND}" | sed "s#'#'\\"'\\"'#g;s#.*#par_sampling_percentile_upper_bound='&'#" ; else echo "# par_sampling_percentile_upper_bound="; fi )
$( if [ ! -z ${VIASH_PAR_SAMPLING_PERCENTILE_STEP+x} ]; then echo "${VIASH_PAR_SAMPLING_PERCENTILE_STEP}" | sed "s#'#'\\"'\\"'#g;s#.*#par_sampling_percentile_step='&'#" ; else echo "# par_sampling_percentile_step="; fi )
$( if [ ! -z ${VIASH_PAR_MIN_INTRON+x} ]; then echo "${VIASH_PAR_MIN_INTRON}" | sed "s#'#'\\"'\\"'#g;s#.*#par_min_intron='&'#" ; else echo "# par_min_intron="; fi )
$( if [ ! -z ${VIASH_PAR_MIN_SPLICE_READ+x} ]; then echo "${VIASH_PAR_MIN_SPLICE_READ}" | sed "s#'#'\\"'\\"'#g;s#.*#par_min_splice_read='&'#" ; else echo "# par_min_splice_read="; fi )
...
$( if [ ! -z ${VIASH_PAR_OUTPUT_PLOT+x} ]; then echo "${VIASH_PAR_OUTPUT_PLOT}" | sed "s#'#'\\"'\\"'#g;s#.*#par_output_plot='&'#" ; else echo "# par_output_plot="; fi )
...
meta["defaults"] = [
  ...
  "container" : {
    "registry" : "ghcr.io",
    "image" : "data-intuitive/rseqc/rseqc_junctionsaturation",
    "tag" : "main_build"
  },
  "tag" : "$id"
}'''),
