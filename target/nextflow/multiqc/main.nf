...
meta = [
  ...
  "description" : "Present QC for raw read, alignment, gene biotype, sample similarity, and strand-specificity checks in a single report.\n",
  "test_resources" : [
    {
      "type" : "bash_script",
      "path" : "test.sh",
    ...
    {
      "type" : "file",
      "path" : "testData/unit_test_resources/data.zip"
    }
  ],
  "info" : {
    "migration_info\\"" : {
  ...
  "build_info" : {
    "config" : "/home/runner/work/rnaseq.vsh/rnaseq.vsh/src/multiqc/config.vsh.yaml",
    "runner" : "nextflow",
    "engine" : "docker",
    "output" : "/home/runner/work/rnaseq.vsh/rnaseq.vsh/target/nextflow/multiqc",
...
$( if [ ! -z ${VIASH_PAR_MULTIQC_CUSTOM_CONFIG+x} ]; then echo "${VIASH_PAR_MULTIQC_CUSTOM_CONFIG}" | sed "s#'#'\\"'\\"'#g;s#.*#par_multiqc_custom_config='&'#" ; else echo "# par_multiqc_custom_config="; fi )
$( if [ ! -z ${VIASH_PAR_MULTIQC_TITLE+x} ]; then echo "${VIASH_PAR_MULTIQC_TITLE}" | sed "s#'#'\\"'\\"'#g;s#.*#par_multiqc_title='&'#" ; else echo "# par_multiqc_title="; fi )
$( if [ ! -z ${VIASH_PAR_MULTIQC_METHODS_DESCRIPTION+x} ]; then echo "${VIASH_PAR_MULTIQC_METHODS_DESCRIPTION}" | sed "s#'#'\\"'\\"'#g;s#.*#par_multiqc_methods_description='&'#" ; else echo "# par_multiqc_methods_description="; fi )
$( if [ ! -z ${VIASH_PAR_MQC_YML+x} ]; then echo "${VIASH_PAR_MQC_YML}" | sed "s#'#'\\"'\\"'#g;s#.*#par_mqc_yml='&'#" ; else echo "# par_mqc_yml="; fi )
$( if [ ! -z ${VIASH_PAR_WORKFLOW_SUMMARY+x} ]; then echo "${VIASH_PAR_WORKFLOW_SUMMARY}" | sed "s#'#'\\"'\\"'#g;s#.*#par_workflow_summary='&'#" ; else echo "# par_workflow_summary="; fi )
...
$( if [ ! -z ${VIASH_PAR_PLOTS+x} ]; then echo "${VIASH_PAR_PLOTS}" | sed "s#'#'\\"'\\"'#g;s#.*#par_plots='&'#" ; else echo "# par_plots="; fi )
