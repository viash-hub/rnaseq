...
meta = [
  ...
  "test_resources" : [
    ...
    {
      "type" : "file",
      "path" : "testData/unit_test_resources/a.sorted.bam"
    }
  ],
  "info" : {
    "migration_info" : {
  ...
  "build_info" : {
    "config" : "/home/runner/work/rnaseq.vsh/rnaseq.vsh/src/bedtools_genomecov/config.vsh.yaml",
    "runner" : "nextflow",
    "engine" : "docker",
    "output" : "/home/runner/work/rnaseq.vsh/rnaseq.vsh/target/nextflow/bedtools_genomecov",
...
$( if [ ! -z ${VIASH_PAR_EXTRA_BEDTOOLS_ARGS+x} ]; then echo "${VIASH_PAR_EXTRA_BEDTOOLS_ARGS}" | sed "s#'#'\\"'\\"'#g;s#.*#par_extra_bedtools_args='&'#" ; else echo "# par_extra_bedtools_args="; fi )
$( if [ ! -z ${VIASH_PAR_VERSIONS+x} ]; then echo "${VIASH_PAR_VERSIONS}" | sed "s#'#'\\"'\\"'#g;s#.*#par_versions='&'#" ; else echo "# par_versions="; fi )
$( if [ ! -z ${VIASH_PAR_BEDGRAPH_FORWARD+x} ]; then echo "${VIASH_PAR_BEDGRAPH_FORWARD}" | sed "s#'#'\\"'\\"'#g;s#.*#par_bedgraph_forward='&'#" ; else echo "# par_bedgraph_forward="; fi )
$( if [ ! -z ${VIASH_PAR_BEDGRAPH_REVERSE+x} ]; then echo "${VIASH_PAR_BEDGRAPH_REVERSE}" | sed "s#'#'\\"'\\"'#g;s#.*#par_bedgraph_reverse='&'#" ; else echo "# par_bedgraph_reverse="; fi )
...
meta["defaults"] = [
  ...
  "container" : {
    "registry" : "ghcr.io",
    "image" : "data-intuitive/bedtools_genomecov",
    "tag" : "main_build"
  },
  "tag" : "$id"
}'''),
