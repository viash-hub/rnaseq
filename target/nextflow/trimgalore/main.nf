...
meta = [
  ...
  "functionality" : {
    ...
    "arguments" : [
      ...
      },
      {
        ...
        "name" : "--trim_log_1",
        "default" : [
          "$id.$key.read1.trimming_report.txt"
        ],
        "must_exist" : true,
        "create_parent" : true,
        "required" : false,
      ...
      },
      {
        ...
        "name" : "--trim_log_2",
        "default" : [
          "$id.$key.read2.trimming_report.txt"
        ],
        "must_exist" : false,
        "create_parent" : true,
        "required" : false,
    ...
    "resources" : [
      {
        ...
        "path" : "script.sh",
        "is_executable" : true,
        "parent" : "file:/home/nirmayi/data_intuitive/rnaseq.vsh/src/trimgalore/"
      }
    ],
    "description" : "A wrapper tool around Cutadapt and FastQC to consistently apply quality and adapter trimming to FastQ files. See https://www.bioinformatics.babraham.ac.uk/projects/trim_galore/ for more infomration.\n",
    "test_resources" : [
      {
        "type" : "bash_script",
        "path" : "test.sh",
  ...
  "info" : {
    ...
    "platform" : "nextflow",
    "output" : "/home/nirmayi/data_intuitive/rnaseq.vsh/target/nextflow/trimgalore",
...
$( if [ ! -z ${VIASH_PAR_TRIM_LOG_1+x} ]; then echo "${VIASH_PAR_TRIM_LOG_1}" | sed "s#'#'\\"'\\"'#g;s#.*#par_trim_log_1='&'#" ; else echo "# par_trim_log_1="; fi )
$( if [ ! -z ${VIASH_PAR_TRIM_LOG_2+x} ]; then echo "${VIASH_PAR_TRIM_LOG_2}" | sed "s#'#'\\"'\\"'#g;s#.*#par_trim_log_2='&'#" ; else echo "# par_trim_log_2="; fi )
$( if [ ! -z ${VIASH_PAR_TRIM_HTML_1+x} ]; then echo "${VIASH_PAR_TRIM_HTML_1}" | sed "s#'#'\\"'\\"'#g;s#.*#par_trim_html_1='&'#" ; else echo "# par_trim_html_1="; fi )
$( if [ ! -z ${VIASH_PAR_TRIM_HTML_2+x} ]; then echo "${VIASH_PAR_TRIM_HTML_2}" | sed "s#'#'\\"'\\"'#g;s#.*#par_trim_html_2='&'#" ; else echo "# par_trim_html_2="; fi )
$( if [ ! -z ${VIASH_PAR_TRIM_ZIP_1+x} ]; then echo "${VIASH_PAR_TRIM_ZIP_1}" | sed "s#'#'\\"'\\"'#g;s#.*#par_trim_zip_1='&'#" ; else echo "# par_trim_zip_1="; fi )
$( if [ ! -z ${VIASH_PAR_TRIM_ZIP_2+x} ]; then echo "${VIASH_PAR_TRIM_ZIP_2}" | sed "s#'#'\\"'\\"'#g;s#.*#par_trim_zip_2='&'#" ; else echo "# par_trim_zip_2="; fi )
$( if [ ! -z ${VIASH_PAR_EXTRA_TRIMGALORE_ARGS+x} ]; then echo "${VIASH_PAR_EXTRA_TRIMGALORE_ARGS}" | sed "s#'#'\\"'\\"'#g;s#.*#par_extra_trimgalore_args='&'#" ; else echo "# par_extra_trimgalore_args="; fi )
$( if [ ! -z ${VIASH_PAR_MIN_TRIMMED_READS+x} ]; then echo "${VIASH_PAR_MIN_TRIMMED_READS}" | sed "s#'#'\\"'\\"'#g;s#.*#par_min_trimmed_reads='&'#" ; else echo "# par_min_trimmed_reads="; fi )
...
meta["defaults"] = [
  ...
  "container" : {
    "registry" : "ghcr.io",
    "image" : "data-intuitive/trimgalore",
    "tag" : "dev"
  },
  "tag" : "$id"
}'''),
