...
meta = [
  ...
  "build_info" : {
    "config" : "/home/runner/work/rnaseq.vsh/rnaseq.vsh/src/stringtie/config.vsh.yaml",
    "runner" : "nextflow",
    "engine" : "docker",
    "output" : "/home/runner/work/rnaseq.vsh/rnaseq.vsh/target/nextflow/stringtie",
...
$( if [ ! -z ${VIASH_PAR_ANNOTATION_GTF+x} ]; then echo "${VIASH_PAR_ANNOTATION_GTF}" | sed "s#'#'\\"'\\"'#g;s#.*#par_annotation_gtf='&'#" ; else echo "# par_annotation_gtf="; fi )
$( if [ ! -z ${VIASH_PAR_EXTRA_STRINGTIE_ARGS+x} ]; then echo "${VIASH_PAR_EXTRA_STRINGTIE_ARGS}" | sed "s#'#'\\"'\\"'#g;s#.*#par_extra_stringtie_args='&'#" ; else echo "# par_extra_stringtie_args="; fi )
$( if [ ! -z ${VIASH_PAR_STRINGTIE_IGNORE_GTF+x} ]; then echo "${VIASH_PAR_STRINGTIE_IGNORE_GTF}" | sed "s#'#'\\"'\\"'#g;s#.*#par_stringtie_ignore_gtf='&'#" ; else echo "# par_stringtie_ignore_gtf="; fi )
$( if [ ! -z ${VIASH_PAR_VERSIONS+x} ]; then echo "${VIASH_PAR_VERSIONS}" | sed "s#'#'\\"'\\"'#g;s#.*#par_versions='&'#" ; else echo "# par_versions="; fi )
$( if [ ! -z ${VIASH_PAR_TRANSCRIPT_GTF+x} ]; then echo "${VIASH_PAR_TRANSCRIPT_GTF}" | sed "s#'#'\\"'\\"'#g;s#.*#par_transcript_gtf='&'#" ; else echo "# par_transcript_gtf="; fi )
$( if [ ! -z ${VIASH_PAR_COVERAGE_GTF+x} ]; then echo "${VIASH_PAR_COVERAGE_GTF}" | sed "s#'#'\\"'\\"'#g;s#.*#par_coverage_gtf='&'#" ; else echo "# par_coverage_gtf="; fi )
$( if [ ! -z ${VIASH_PAR_ABUNDANCE+x} ]; then echo "${VIASH_PAR_ABUNDANCE}" | sed "s#'#'\\"'\\"'#g;s#.*#par_abundance='&'#" ; else echo "# par_abundance="; fi )
$( if [ ! -z ${VIASH_PAR_BALLGOWN+x} ]; then echo "${VIASH_PAR_BALLGOWN}" | sed "s#'#'\\"'\\"'#g;s#.*#par_ballgown='&'#" ; else echo "# par_ballgown="; fi )
...
meta["defaults"] = [
  ...
  "container" : {
    "registry" : "ghcr.io",
    "image" : "data-intuitive/stringtie",
    "tag" : "main_build"
  },
  "tag" : "$id"
}'''),
